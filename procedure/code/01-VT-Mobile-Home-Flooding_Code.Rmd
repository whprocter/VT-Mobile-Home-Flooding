...
packages <- c("downloader", "rdhs", "haven", "readr", "tidyverse", "pastecs",
             ...
             "tmap", "units", "knitr", "tibble", "here", "dplyr")
...
## And do the same for river corridors
```{r}
...
## Add county variables to the mobile homes
```{r}
mh_with_county <- st_join(e911_mh, block_groups, join = st_intersects)


# Group MHs by county and sum them for each county to get column 1
...
## Add buffer to mobile home points
```{r}
mh_buffer <- st_buffer(mh_with_county, dist = 18.3) #18.3 meters is the average length of a mobile home
```

## Identify the mobile homes within FEMA flood zone
```{r}
MHs_in_flood_zone <- st_join(mh_buffer, FEMA_multipart, join = st_intersects)

MHs_in_flood_zone <- MHs_in_flood_zone %>%
  dplyr::filter(flood == TRUE)
```

```{r}
MHs_in_flood_zone_by_county <- MHs_in_flood_zone %>%
  group_by(county) %>%
  count(county, name = "mobile_home_count") %>%
  summarise(mobile_home_count = n())
```
...
## Identify the mobile homes within river corridors
```{r}
MHs_in_river_corridors <- st_join(mh_buffer, river_corridors_multipart, join = st_intersects)

MHs_in_river_corridors <- MHs_in_river_corridors %>%
  dplyr::filter(flood == TRUE)

```

```{r}
MHs_in_river_corridor_by_county <- MHs_in_river_corridor %>%
  group_by(county) %>%
  count(county, name = "mobile_home_count") %>%
  summarise(mobile_home_count = n())
